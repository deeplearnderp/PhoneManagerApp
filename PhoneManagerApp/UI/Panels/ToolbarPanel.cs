...
{
    private readonly ScreenMirrorService _screenMirrorService = new();
    ...
    private void InitializeControls()
    {
        ...
        // 🔘 Buttons
        // ================================
        BtnConnectPhone = CreateButton("Connect Phone", 490, 110,
            (_, _) => ConnectPhoneClicked?.Invoke(this, EventArgs.Empty));

        BtnToggleNotifications = CreateButton("Toggle Notifications", 610, 140,
            (_, _) => ToggleNotificationsClicked?.Invoke(this, EventArgs.Empty));

        BtnShowTerminal = CreateButton("Hide Terminal", 760, 110,
            (_, _) => ToggleTerminalClicked?.Invoke(this, EventArgs.Empty));

        BtnClearOutput = CreateButton("🖋 Clear Output", 880, 120,
            (_, _) => ClearOutputClicked?.Invoke(this, EventArgs.Empty));
